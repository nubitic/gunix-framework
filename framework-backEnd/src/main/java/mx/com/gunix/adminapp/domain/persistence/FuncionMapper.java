...
public interface FuncionMapper {
	@Select("Select ID_APLICACION, ID_MODULO, ID_FUNCION, TITULO, DESCRIPCION, PROCESS_KEY, ORDEN, ID_FUNCION_PADRE, VENGINE::text AS VENGINE  from SEGURIDAD.FUNCION where ID_APLICACION=#{idAplicacion} AND ID_MODULO=#{idModulo} ORDER BY ORDEN, TITULO")
