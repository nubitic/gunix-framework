...
public final class EmbeddedPostgreSQLManager {
	...
	public static void start(String pgsqlHome, String usuario, String password, String database, String puerto, ClassLoader classLoader) {
		...
		if (!existeEsquema(pgsqlHomeFile, usuario, database, puerto, "seguridad")) {
	...
	private static void ejecutaAppScripts(File pgsqlHomeFile, String usuario, String database, String puerto) {
		try {
			...
			if (appScriptsResources != null) {
				...
				// Los scripts para las funciones se agregan al final para que siempre se
				// ejecuten al último, planchanco cualquier versión definida previamente en los
				// SQLs normales.
				...
				for (Resource appScriptResource : currentAppScriptsResourcesList) {
					...
					if (appScriptResource.getFilename().toLowerCase().endsWith(".zip")) {
						ZipEntryWorker.withAllZipEntries(appScriptResource.getInputStream(), (sqlFileName, sqlIs) -> {
							ejecutaScriptInn(sqlIs, pgsqlHomeFile, usuario, database, puerto, appScriptResource.getFilename(), writer);
						});
					} else {
						ejecutaScriptInn(appScriptResource.getInputStream(), pgsqlHomeFile, usuario, database, puerto, appScriptResource.getFilename(), writer);
					}
					writer.close();
				}
			}
	...
	private static void ejecutaScriptInn(InputStream scriptIS, File pgsqlHomeFile, String usuario, String database, String puerto, String fileName, BufferedWriter writer) {
		ejecutaScript(scriptIS, pgsqlHomeFile, usuario, database, puerto, new Logger(fileName) {
			@Override
